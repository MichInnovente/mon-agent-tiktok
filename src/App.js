  ...
  useEffect(() => {
    const fetchAgents = async () => {
      try {
        ...
        // SÉCURITÉ : On vérifie que data.agents est bien une liste (Array)
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {agents.length > 0 ? (
            ...
            <p className="col-span-3 text-center text-gray-500 italic">Connexion au Sheets en cours...</p>
          )}
        </div>
        ...
        {result && <div className="mt-8 p-6 bg-green-50 text-green-700 rounded-2xl border border-green-200 text-center">{result}</div>}
      </div>
    </div>
  );
}
